  ...
  *offset=0;
  ...
  while(!fscantoken(file.file.file,key))
    ...
    else if(!strcmp(key,"cellsize"))
    {
      ...
      if(fscanfloat(&file,&header->cellsize_lat,"cellsize_lat",FALSE, VERB ))
      {
        free(name);
        fclose(file.file.file);
        return NULL;
    ...
    {
      if(fscanstring(&file,value,"datatype",FALSE, VERB ))
      {
        if(isout)
    ...
    {
      if(fscanstring(&file,value,"endian",FALSE, VERB ))
      {
        if(isout)
    ...
    {
      if(fscanstring(&file,value,"file",FALSE, VERB ))
      {
        if(isout)
          readstringerr("file");
        free(name);
    ...
    {
      if(fscanstring(&file,value,"remark",FALSE, VERB ))
      {
        if(isout)
